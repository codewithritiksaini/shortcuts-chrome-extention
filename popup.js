...
async function loadShortcutsList() {
  ...
  if (count === 0) {
    list.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-state-text">No shortcuts created yet</div>
        <div class="empty-state-hint">Create your first shortcut above</div>
      </div>
    `;
    return;
  }
  ...
  for (const [key, shortcut] of Object.entries(shortcuts)) {
    ...
    // Display emojis as a preview
    const emojisPreview = shortcut.emojis ? 
      `<div class="shortcut-emojis">${shortcut.emojis}</div>` : 
      '<div class="no-emojis">No emojis</div>';
    
    item.innerHTML = `
      <div class="shortcut-header">
        <div class="shortcut-command">${key}</div>
        <div class="shortcut-actions">
          <button class="icon-btn edit" data-key="${key}" title="Edit shortcut">
            ✏️
          </button>
          <button class="icon-btn delete" data-key="${key}" title="Delete shortcut">
            🗑️
          </button>
        </div>
      </div>
...
async function syncWithGitHub() {
  ...
  try {
    ...
    await chrome.storage.sync.set({ shortcuts: merged });
...
// Delete confirmation modal - UPDATED WITH GITHUB CHECKBOX
function showDeleteConfirmation(key) {
  ...
  modal.innerHTML = `
    ...
    <div class="modal-body">
      <p>Are you sure you want to delete <strong>${key}</strong>?</p>
      
      ${githubSync && syncSettings.enabled ? `
      <div class="form-group" style="margin-top: 15px;">
        <div class="checkbox">
          <input type="checkbox" id="deleteFromGitHub">
          <label for="deleteFromGitHub">Also delete from GitHub</label>
        </div>
        <div class="form-hint">Uncheck to keep this shortcut on GitHub</div>
      </div>
      ` : ''}
    ...
    <div class="modal-actions">
      ...
      <button class="modal-btn modal-btn-confirm" data-key="${key}">Delete</button>
    </div>
  `;
...
// Clear all shortcuts with confirmation - UPDATED
async function clearAllShortcuts() {
  ...
  modal.innerHTML = `
    <div class="modal-header">
      <div class="modal-title">Clear All Shortcuts</div>
    </div>
    <div class="modal-body">
      <p>Are you sure you want to delete <strong>all ${Object.keys(shortcuts).length} shortcuts</strong>?</p>
      
      ${syncSettings.enabled ? `
      <div class="form-group" style="margin-top: 15px;">
        <div class="checkbox">
          <input type="checkbox" id="clearFromGitHub" checked>
          <label for="clearFromGitHub">Also clear from GitHub</label>
        </div>
        <div class="form-hint">Uncheck to keep shortcuts on GitHub</div>
      </div>
      ` : ''}
    ...
    <div class="modal-actions">
      <button class="modal-btn modal-btn-cancel">Cancel</button>
      <button class="modal-btn modal-btn-confirm">Clear All</button>
    </div>
  `;
  ...
  modal.querySelector('.modal-btn-confirm').addEventListener('click', async () => {
    ...
    // Clear from GitHub if checkbox checked and connected
    ...
    showStatus('All shortcuts cleared' + (clearFromGitHub ? ' from local and GitHub' : ' locally'), 'success');
    overlay.remove();
    loadShortcutsList();
  });
}
